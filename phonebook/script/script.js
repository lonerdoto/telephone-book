...

{
    ...
    const createFormInfo = () => {
        ...
        if (editButton.getAttribute('listener') !== 'true') {
            ...
            editButton.addEventListener('click', () => {
                ...
                editContactBtns = document.querySelectorAll('.edit-button');
                editContactBtns.forEach(el => {
        ...
        for (let i = 0; i < btnsEdit.length; i++) {
            btnsEdit[i].addEventListener('click', () => {
                ...
                arr['name'] = formName.value;
                arr['surname'] = formSurname.value;
                arr['phone'] = formPhone.value;

                if (JSON.stringify(arr) == JSON.stringify(data[i])) {
                    index = i;
                };
    ...
    const createFooter = (title) => {
        ...
        footerContainer.insertAdjacentHTML('beforeend', `<span>Все права ${title.substr(6).bold()} защищены &copy; </span>`)
    ...
    const hoverRow = (allRow, logo) => {
        ...
        allRow.forEach(contact => {
            contact.addEventListener('mouseenter', () => {
                logo.textContent = contact.childNodes[2].textContent + ' ' + contact.childNodes[1].textContent + ' : ' + contact.phoneLink.textContent;
            });
    ...
    const deleteControl = (btnDel, list) => {
        ...
        list.addEventListener('click', e => {
            ...
            if(target.closest('.del-icon')){
                ...
                let number = target.closest('.contact').childNodes[3].childNodes[0].innerText
                clearStorage(number);
            }
        });
    };
    ...
    const setStorage = () => {
        let dataStorage = localStorage.setItem('data', JSON.stringify(data));
        return dataStorage;
    }
    ...
    const clearStorage = (number) => {
        for(let i = 0; i < JSON.parse(localStorage.getItem('data')).length; i++) {
            if (data[i].phone == number) {
                data.splice(i, 1);
                setStorage();
